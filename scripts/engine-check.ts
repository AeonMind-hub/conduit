...
t("one run is bounded in documents, bytes and characters",
  /* Twenty documents, not twelve: a run has to be able to hold a whole reconciliation — order, invoice,
     receipt, the invoice again, a booking and the noise around them — or the bound is describing a demo
     rather than a day. The number is still what makes an open endpoint safe, and still asserted as such. */
  MAX_PILOT_DOCS === 20 && MAX_PILOT_BYTES === 1_500_000 && PILOT_CHARS_PER_DOC === 12_000,
...
/* ── the batch layer: lines, tolerance, duplicates, policy ───────────────────────────────────────
   Each assertion below is a promise the page makes about a *set* of documents. The samples are the
   batch the demo runs, so their captions and the engine's answers are tied together here: if the
   invoice stops being held on the variance, or the duplicate stops being caught, the chip is lying. */
import { runBatch, policyFrom } from "../src/lib/batch";
import { BATCH } from "../src/lib/samples";

{
  const inputs = BATCH.map((s, i) => ({ i, name: `${s.key}.txt`, bytes: s.body.length, kind: "text", text: s.body }));
  const run = runBatch(inputs, {});
  for (const [k, s] of BATCH.entries()) {
    const d = run.docs[k];
    t(`sample「${s.label}」 lands where its caption promises`,
      d.type === s.expect.type && d.status === (s.expect.outcome === "held" ? "exception" : s.expect.outcome)
      && (s.expect.flag == null || d.flags.includes(s.expect.flag)),
      `${d.type}/${d.status}/[${d.flags}]`);
  }
  const g = run.groups.find(x => x.key === "PO:88241");
  t("the batch reconciles to a 5% quantity variance, stated as a delta against the policy",
    g?.verdict === "variance" && g.ordered === 720 && g.billed === 684 && /5\.0% over a 2% tolerance/.test(g.sentence ?? ""),
    JSON.stringify({ v: g?.verdict, s: g?.sentence }));
  t("three documents in the batch clear, two are held, one is set aside — and only the bill is held",
    run.counts.committed === 3 && run.counts.held === 2 && run.counts.discarded === 1
    && run.docs[0].status === "committed" && run.docs[2].status === "committed", JSON.stringify(run.counts));
  const wide = runBatch(inputs, { tolerancePct: 10 });
  t("the same batch under a 10% tolerance commits: policy moves rows, the engine does not opine",
    wide.docs[1].status === "committed" && wide.groups.find(x => x.key === "PO:88241")?.verdict === "matched",
    `${wide.docs[1].status}/${wide.docs[1].flags}`);

  const derived = runBatch([{ i: 0, name: "l.txt", bytes: 0, kind: "text",
    text: "SUPPLIER INVOICE\nInvoice No: D-1\nLine 1 SKU BS-1140\nQuantity: 1,000  Unit price: £2.50" }], {});
  const ln = derived.docs[0].lines[0];
  t("a line with no stated amount is derived by arithmetic and marked as derived",
    ln.amount === 2500 && ln.derived === true && ln.confidence <= 0.9, JSON.stringify(ln));

  const disagree = runBatch([{ i: 0, name: "t.txt", bytes: 0, kind: "text",
    text: "SUPPLIER INVOICE\nInvoice No: D-2\nQuantity: 1,000  Unit price: £2.50  Line total: £2,500.00\nAmount due: £2,700.00" }], {});
  t("a stated total that disagrees with the lines holds the row on the difference",
    disagree.docs[0].totals.variance !== null && Math.abs(Math.abs(disagree.docs[0].totals.variance!) - 200) < 0.005
    && disagree.docs[0].status === "exception", JSON.stringify(disagree.docs[0].totals));

  const pol = policyFrom({ tolerancePct: 90, threshold: 0.5, autoApproveUnder: -5 });
  t("an impossible policy is clamped into range and the clamp is reported, not swallowed",
    pol.policy.tolerancePct === 25 && pol.policy.threshold >= 0.6 && pol.clamped.length === 3,
    JSON.stringify(pol));
}
