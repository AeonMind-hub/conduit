...
try {
  ...
  check("the page is an input before it is a pitch",
    /<textarea/.test(root.html) && root.visible.indexOf("Your whole dock,") < root.visible.indexOf("run one at a time:"),
    `textarea ${/textarea/.test(root.html)}`);
  check("there is exactly one input on the page", (root.html.match(/<textarea/g) || []).length === 1);
  check("the headline is the promise, and it is about a batch",
    /Your whole dock,/.test(root.visible) && /on your own paper\./.test(root.visible)
    && /checks them against each other/.test(root.visible.replace(/\s+/g, " ")));
  ...
  check("the run button is a verb, not a tour", /Run it/.test(root.visible) && /or choose files/.test(root.visible));
  check("a batch can be run without supplying anything, in one click", /Run the sample batch/.test(root.visible));
  check("the client's policy is on the page, because it changes the answer",
    /Variance tolerance, percent/.test(root.visible) && /Nothing posts before goods are received/.test(root.visible)
    && /Above this, a person approves even if the machine is happy/.test(root.visible));
  check("the three sides of a three-way match are named", /ordered/.test(root.visible) && /billed/.test(root.visible)
    && /received/.test(root.visible));
  ...
  check("a phone is not told to press a keyboard shortcut",
    /⌘↵/.test(root.visible), "the keyboard shortcut is advertised");
  ...
  check("the page says how many documents a run takes", /up to 20 documents/.test(root.visible));
  check("the disclosure is exactly one, and says what is not done here",
    (root.visible.match(/One disclosure/g) ?? []).length === 1
    && /is not OCR/.test(root.visible.replace(/\s+/g, " ")) && /no key is asked for/.test(root.visible.replace(/\s+/g, " ")));
  ...
  check("the page names other workflows the same build fits, not just this one",
    /not the only workflow this fits/i.test(root.visible) && /onboarding/i.test(root.visible)
    && /approval chains/i.test(root.visible));
  check("there is a way to start a conversation about a different workflow",
    /describe your workflow/i.test(root.visible));
  check("the page identifies who built it, and offers a way to reach them, before the headline",
    /Nathaniel Zinsu/.test(root.visible) && root.visible.indexOf("Nathaniel Zinsu") < root.visible.indexOf("Your whole dock,")
    && /get in touch/i.test(root.visible));
  ...
  // ── 2b. the batch itself: three-way matching, tolerance as policy, duplicates ──
  {
    const INVS = ["MERIDIAN FASTENERS LTD", "SUPPLIER INVOICE", "Invoice No: MF-9930", "PO Number: 88241",
      "Invoice date: 2026-09-03", "Line 1  SKU BS-1140  Hex bolt M10x40 zinc",
      "Quantity: 3,800   Unit price: £0.18   Line total: £684.00", "Amount due: £684.00"].join("\n");
    const GRN = ["GOODS RECEIVED NOTE GRN-4410", "Received into store at Dock 4", "PO reference: 88241",
      "SKU BS-1140", "Quantity received: 4,000 units", "Received on: 2026-09-28"].join("\n");
    const form = (tol, extra) => {
      const f = new FormData();
      f.append("files", new Blob([PO], { type: "text/plain" }), "po.txt");
      f.append("files", new Blob([INVS], { type: "text/plain" }), "invoice.txt");
      f.append("files", new Blob([GRN], { type: "text/plain" }), "grn.txt");
      (extra ?? []).forEach(([n, t]) => f.append("files", new Blob([t], { type: "text/plain" }), n));
      f.append("policy", JSON.stringify({ tolerancePct: tol }));
      return f;
    };
    const tight = await post(form(2));
    const inv = byName(tight.events, "invoice.txt");
    check("an invoice that bills less than the order is held on the delta, never corrected",
      inv?.status === "exception" && /5\.0% over a 2% tolerance/.test(inv?.note ?? ""), (inv?.note ?? "none").slice(0, 64));
    check("the order and the receipt are not blamed for the invoice's variance",
      byName(tight.events, "po.txt")?.status === "committed" && byName(tight.events, "grn.txt")?.status === "committed");
    check("the arithmetic is per line, in both currencies of the batch",
      inv?.lines?.[0]?.amount === 684 && byName(tight.events, "po.txt")?.lines?.[0]?.amount === 720,
      JSON.stringify({ i: inv?.lines?.[0]?.amount, p: byName(tight.events, "po.txt")?.lines?.[0]?.amount }));
    const grp = tight.events.find(e => e.t === "matches")?.groups?.[0];
    check("the match names both sides of the difference",
      grp?.verdict === "variance" && grp?.ordered === 720 && grp?.billed === 684,
      JSON.stringify({ v: grp?.verdict, o: grp?.ordered, b: grp?.billed }));
    check("a receipt is read as a receipt, including its own date label",
      (byName(tight.events, "grn.txt")?.fields ?? []).some(f => f.key === "received_date" && f.value === "2026-09-28"));
    check("a row carries the steps it went through", (inv?.audit ?? []).length >= 4
      && (inv?.audit ?? []).some(a => a.kind === "held"), `${(inv?.audit ?? []).length} events`);
    check("what would be written carries a key over the document, not the filename",
      /^88241:[0-9a-f]{16}$/.test(String(inv?.duplicateOf != null ? "" : byName(tight.events, "po.txt")?.payload?.idempotency_key ?? "")),
      String(byName(tight.events, "po.txt")?.payload?.idempotency_key ?? "none"));
    const loose = await post(form(10));
    check("tolerance is policy: the same batch with a wider one commits",
      byName(loose.events, "invoice.txt")?.status === "committed"
      && loose.events.find(e => e.t === "matches")?.groups?.[0]?.verdict === "matched",
      `${byName(loose.events, "invoice.txt")?.status}`);
    const twice = await post(form(2, [["invoice-forwarded-again.txt", INVS]]));
    const dup = byName(twice.events, "invoice-forwarded-again.txt");
    check("one document forwarded twice is still one document",
      dup?.status === "exception" && (dup?.flags ?? []).includes("DUPLICATE_DOCUMENT") && dup?.payload == null,
      JSON.stringify(dup?.flags ?? []));
    const apForm = new FormData();
    apForm.append("files", new Blob([INVS], { type: "text/plain" }), "paid-invoice.txt");
    apForm.append("policy", JSON.stringify({ tolerancePct: 90 }));
    const clamped = await post(apForm);
    check("an absurd tolerance is clamped, and the page is told it was",
      /tolerancePct/.test(JSON.stringify(clamped.events.find(e => e.t === "done")?.clamped ?? [])),
      JSON.stringify(clamped.events.find(e => e.t === "done")?.clamped ?? []));
  }
  ...
  check("a PDF with a text layer carries the same identity as the same words pasted",
    byName(run1.events, "northgate-po.pdf")?.digest === byName(run1.events, "a purchase order · our sample")?.digest
    && byName(run1.events, "northgate-po.pdf")?.type === "purchase_order");
  ...
  {
    const a = byName(run1.events, "northgate-po.pdf")?.digest;
    const b = byName(run1.events, "northgate-po (1).pdf")?.digest;
    const c = byName(run1.events, "northgate-po-revised.pdf")?.payload?.idempotency_key;
    check("the same document sent twice under two names is one document, and only one row is written",
      !!a && a === b && byName(run1.events, "northgate-po (1).pdf")?.status === "exception"
      && (byName(run1.events, "northgate-po (1).pdf")?.flags ?? []).includes("DUPLICATE_DOCUMENT"), `${a} vs ${b}`);
    ...
    check("the key is the customer's own reference, not our row number",
      byName(run1.events, "a purchase order · our sample")?.payload?.external_id === "88241");
    check("the payload carries provenance, so a row can be audited back to a document",
      byName(run1.events, "a purchase order · our sample")?.payload?.provenance?.engine === "rules");
  }
  {
  ...
  // ── 3. bounds: what makes an open endpoint safe ──────────────────────────────
  {
    ...
    for (let k = 0; k < 21; k++) many.append("files", new Blob([PO], { type: "text/plain" }), `d${k}.txt`);
    const big13 = await post(many);
    check("twenty-one documents are refused by our bound, not the platform's",
      big13.status === 413 && /20 documents/.test(big13.text), `${big13.status}`);
  ...
  // ── 4. /systems is generated from the same code, not from a slide ────────────
  {
    ...
    /* The engine is the field rules *and* the batch layer now: three-way matching, tolerance and duplicates
       are gates like any other, so they belong in the set of codes the page is allowed to name. */
    const src = ["src/lib/types.ts", "src/lib/rules.ts", "src/lib/batch.ts", "src/app/api/pilot/route.ts", "src/lib/intake.ts"]
  ...
  // ── 4b. /run on its own, with nothing handed to it, says so honestly ─────────
  {
    const run = await get("/run");
    check("GET /run is 200", run.status === 200, `status ${run.status}`);
    check("a cold /run — no pending run, no session — reads as empty, not broken",
      /Nothing is running/.test(run.visible) && /paste a document/i.test(run.visible));
  }
  ...
  // ── 6. the stylesheet that shipped, and the marks it draws ───────────────────
  {
    ...
    const resultsView = readFileSync("src/components/ResultsView.tsx", "utf8");
    check("a confidence bar is drawn from the number, not from a caption",
      /<i style=\{\{[\s\S]{0,200}?confidence/.test(resultsView), "the fill's width comes from the number");
    const m = resultsView.match(/className=\{?`?meter[^`]*`?\}?>([\s\S]{0,160}?)<\/span>/);
