...
export function findCompany(text: string, labels: string, notOn = ""): Hit {
  ...
  // Greedy over consecutive suffix words, not just one: a real company name is often two of them in a
  // row ("Industrial Ltd", "Trading Group Ltd"), and matching only the first drops the entity type off
  // the end of the name — which is exactly the part that distinguishes it from a similarly-named company.
...
export function findAddress(text: string, labels = "ship to|ship-to|deliver to|delivery address|site|address"): Hit {
  const lines = flat(text).split("\n");
  const labelAlone = new RegExp(String.raw`^(?:${labels})\s*:?\s*$`, "i");
  const labelSameLine = new RegExp(String.raw`^(?:${labels})\s*[:\-]\s*(.+)$`, "i");
  // A line that itself reads as "Some Other Label: value" ends the address block — it belongs to the
  // next field, not one more line of this address.
  const looksLikeNewLabel = /^[A-Za-z][A-Za-z /]{2,28}\s*[:\-]\s*\S/;
  for (let i = 0; i < lines.length; i++) {
    const t = lines[i].trim();
    const same = labelSameLine.exec(t);
    if (same) return { value: sentence(same[1].trim()).slice(0, 110), confidence: 0.95, evidence: t.slice(0, 90) };
    if (!labelAlone.test(t)) continue;
    const first = (lines[i + 1] ?? "").trim();
    if (!first || first.length < 2) continue;
    // Only reached when the label had nothing on its own line — the address is on what follows, and
    // real addresses commonly wrap onto a second line (street, then city + postcode). flat() has
    // already collapsed blank lines by this point, so a second line is only added when it doesn't
    // itself look like an unrelated labelled field or a signature sitting right under the address.
    const parts = [first];
    const second = (lines[i + 2] ?? "").trim();
    if (second && !looksLikeNewLabel.test(second) && /\d/.test(second)) parts.push(second);
    const value = parts.join(", ");
    return { value: sentence(value).slice(0, 110), confidence: 0.9,
      evidence: [t, ...parts].join(" / ").slice(0, 90) };
  }
...
const SIGNALS: Record<string, Sig[]> = {
  ...
  /* Narrow on purpose: a booking note that says "delivered" must not be read as a receipt, or a committed
     row moves lanes for a reason nobody can follow. So the type has to name itself a receipt, or pair
     "received" with a quantity or a date. */
  goods_receipt: [
    { re: /goods\s+received|\bgrn\b|receipt\s+note|goods\s*in\s*note/i, w: 3.4 },
    { re: /quantity\s+received|qty\s+received|received\s+qty|pallets?\s+received/i, w: 2.4 },
    { re: /received\s+(?:on|into|at|by)\b/i, w: 1.6 },
    { re: /\bpo\s*(?:reference|ref)\b/i, w: 1.2 },
    { re: /carrier|driver|vehicle|dock\s*\d|loading bay/i, w: 0.7 },
  ],
};
...
export function classify(text: string): Classified {
  ...
  /* A document says what it is in its heading, and a supplier who writes "SUPPLIER INVOICE" at the top has
     answered the question outright. The heading decides *which* type wins — an invoice quoting "PO Number",
     item lines and unit prices (which is what a real invoice looks like) otherwise scores as a purchase
     order and has its fields read by the wrong rule set.

     What it does not buy is confidence. The certainty a row prints is computed from the document's own
     vocabulary underneath, so a document that is genuinely half quote and half order stays ambiguous and
     stays held instead of being talked across the gate by its title. */
  const head = t.slice(0, 96);
  const TITLE: Record<string, RegExp> = {
    purchase_order: /purchase order/i,
    supplier_invoice: /\binvoice\b|statement of account/i,
    goods_receipt: /goods received|\bgrn\b|receipt note/i,
    delivery_booking: /\bbooking\b|collection note|delivery note/i,
    quote_request: /\bquote\b|quotation|request for quotation/i,
  };
  const bonusFor = (type: string) => (TITLE[type] && TITLE[type].test(head) ? 1.6 : 0);
  const scored = Object.entries(SIGNALS).map(([type, sigs]) => {
    const hits = sigs.filter(s => s.re.test(t));
    const base = hits.reduce((a, h) => a + h.w, 0);
    const bonus = bonusFor(type);
    return { type: type as DocType, s: base + bonus, base,
      hits: bonus ? [...hits.map(h => h.re.source.slice(0, 28)), "named in the heading"] : hits.map(h => h.re.source.slice(0, 28)) };
  }).sort((a, b) => b.s - a.s);
  ...
  /* Every decision below reads `base` — the document's own vocabulary — so the heading can pick a winner
     but cannot manufacture certainty, and cannot beat the noise floor on its own. */
  if (!top || top.base < 1.2)
  ...
  const strength = Math.min(1, top.base / 6);
  const margin = second && second.base > 0 ? (top.base - second.base) / top.base : 1;
...
const RULES: Record<string, (t: string, ctx: Ctx) => Hit> = {
  ...
  "purchase_order.contact": t => findAfterLabel(t, "contact|attn|attention", { max: 60 }),
  ...
  /* A receipt's own date label. `goods received` is left out on purpose: it appears in the heading, and a
     label that also titles the document captures the characters after the title — no date — and answers
     nothing. */
  "goods_receipt.received_date": t => findDate(t, "received on|received date|date received"),
  "goods_receipt.grn_number": t => findRef(t, "any", { allowShape: false }),
  "goods_receipt.po_ref": t => findRef(t, "po"),
  "goods_receipt.qty_received": t => findNumber(t, "quantity received|qty received|received qty|pallets received", "units?|pcs|pallets"),
  "goods_receipt.sku": t => findSku(t),
  "goods_receipt.carrier": t => findCarrier(t),
  "goods_receipt.site": t => findAfterLabel(t, "site|store|warehouse|depot", { max: 64 }),
};
